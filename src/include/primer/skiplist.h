...
class SkipList {
 ...
 public:
  ...
  explicit SkipList(const Compare &compare = Compare{}) {
    compare_ = compare;
    // 开始就设定 head_ 高度为 MaxHeight，因为SkipNode的高度是固定的，之后不能改变
...
SKIPLIST_TEMPLATE_ARGUMENTS struct SkipList<K, Compare, MaxHeight, Seed>::SkipNode {
   ...
   */
  // note: K{}为调用K的默认构造函数
  // 这里的key是一个默认参数，表示如果没有传入key，则使用默认构造函数创建一个key
  explicit SkipNode(size_t height, K key = K{})
      : links_(std::vector<std::shared_ptr<SkipNode>>(height)), key_(std::move(key)) {}
